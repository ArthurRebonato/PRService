...
export default function EditarVagaPage(props) {
    ...
    const efetuarAlteracoes = async() => {
        if (form.nome && form.local && form.modo && form.experiencia && form.requisito && 
          ...
          try {
            ...
            const updatedForm = {...form, nomeLower: `${form.nome}`.toLowerCase()};
            setForm(updatedForm);

            await VagasService.updateVaga(form.key, updatedForm);
