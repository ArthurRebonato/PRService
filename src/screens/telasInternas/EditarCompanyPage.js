...
import { uploadToFirebase, getImageFromFirebase, deleteImageFromFirebase } from '../../services/ImagemService';

export default function EditarCompanyPage(props) {
    ...
    const handleImagePicker = async () => {
        try {
            ...
            if (!result.canceled) {
                setForm((prevForm) => ({ ...prevForm, image: result.assets[0].uri }));
    ...
    const handlePhoto = async () => {
        try {
            ...
            if (!cameraResp.canceled) {
                setForm((prevForm) => ({ ...prevForm, image: cameraResp.assets[0].uri }));
    ...
    const efetuarAlteracoes = async() => {
      if (image != "") {
        if (form.nome && form.email && form.sede && form.fundadores && form.fundacao && form.atuacao && form.descricao && form.site && form.empresas && form.empregados) {
          try {
            setLoading(true);

            if (emailAntigo !== form.email) {
                await deleteImageFromFirebase(`${emailAntigo}-perfilCompany`)
            }

            await CompanyService.updateCompany(keyDoc, form);

            const fileName = `${form.email}-perfilCompany`;
            await uploadToFirebase(form.image, fileName);
